...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fImpParams = new FragmentImpParams();
        fLoadMonitor = new FragmentLoadMonitor();
        //Открываем страницу поиска
        FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
        transaction.replace(R.id.container, fSearch);
        transaction.commit();
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_search) {
            ...
            transaction.replace(R.id.container, fMainParams);
        } else if (id == R.id.nav_time) {
            transaction.replace(R.id.container, fMainParams);
        } else if (id == R.id.nav_imp_1) {
            transaction.replace(R.id.container, fImpParams);
        } else if (id == R.id.nav_imp_2) {
            transaction.replace(R.id.container, fImpParams);
        } else if (id == R.id.nav_load_monitor) {
            transaction.replace(R.id.container, fLoadMonitor);
        }
